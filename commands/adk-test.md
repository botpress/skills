...
2. **Build an invocation appropriate for the type.** If the user provided input after the primitive name, use it as-is; otherwise generate a realistic probe from the primitive's schema:
   ...
   - **Action:** invoke through `adk run scripts/test-<name>.ts` after writing a small disposable runner under `scripts/`, or via a chat probe that triggers a tool which calls the action. Prefer the runner when the action has a deterministic input shape.
...
Clean up any disposable runner scripts under `scripts/test-*.ts` after the test unless the user asks to keep them.
