...
5. **Static checks** (read the sources and verify):
   ...
   - Any referenced integration is installed (`adk integrations list --format json`) and ready (`adk integrations status --format json`; configure with `adk integrations configure` if not).
