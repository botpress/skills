...
6. **Wrap up.** Link the user to the Dev Console (control panel) if a URL is in the deploy output. If the ship fixed a bug, point the user at `/adk-eval` to capture a regression eval. If the deploy changed user-facing behavior, point them at `/schedule` to set up a soak check.
