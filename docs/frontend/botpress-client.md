...
- `upsertTableRow()` - Create or update
- `deleteTableRow()` - Delete by ID
...
```typescript
import type { Client } from "./client.gen"; // Generated by adk dev
...
export const getZaiClient = (workspaceId: string, botId: string) => {
  ...
  const client = new APIClient({
    ...getApiClient({ workspaceId, botId }).config, // Reuse existing config
